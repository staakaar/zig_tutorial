...
test "hex" {
    ...
    try expectEqualStrings("0xAAAAAA", try bufPrint(&b, "0x{X}", .{4294967294}));

    try expectEqualStrings("5a696721", try bufPrint(&b, "{}", std.fmt.fmtSliceHexLower("Zig!")));
}
...
test "B Bi" {
    ...
    try expectEqualStrings("1.024kB", try bufPrint(&b, {}, .{fmtIntSizeBin(1024)}));
    try expectEqualStrings("1KiB", try bufPrint(&b, {}, .{fmtIntSizeBin(1024)}));

    try expectEqualStrings("1.073741824GB", try bufPrint(&b, {}, .{fmtIntSizeBin(1024 * 1024 * 1024)}));
